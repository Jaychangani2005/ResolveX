...
export default function NGOReportsScreen() {
  ...
  const loadIncidents = async () => {
    try {
      setLoading(true);
      console.log('🏛️ Loading incidents for NGO view...');
  ...
  const handleIncidentPress = (incident: IncidentReport) => {
    ...
    router.push(`/incident-details/${incident.id}`);
  };
          ...
          {filteredIncidents.length > 0 ? (
            ...
            <View style={styles.emptyContainer}>
              ...
              <Text style={[styles.emptyText, { color: colors.secondary }]}>
                {searchQuery ? 'No reports found' : 'No incident reports yet'}
              </Text>
              <Text style={[styles.emptySubtext, { color: colors.secondary }]}>
                {searchQuery ? 'Try adjusting your search' : 'Reports will appear here once submitted'}
              </Text>
            </View>
          )}
        </View>
